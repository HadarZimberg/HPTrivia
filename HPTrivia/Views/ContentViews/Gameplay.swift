...
struct Gameplay: View {
    ...
    @State private var tappedWrongAnswer: [String] = []
        ...
        GeometryReader { geo in
            ZStack {
                Image(.hogwarts)
                    ...
                    .overlay {
                        Rectangle()
                            .foregroundStyle(.btnForeStyle.opacity(0.6))
                    }
                VStack {
                    // MARK: Controls
                    HStack {
                                ...
                                if answer == game.currentQuestion.answer {
                                            ...
                                            if !tappedCorrectAnswer {
                                                Button {
                                                    ...
                                                    Text(answer)
                                                        ...
                                                        .background(tappedCorrectAnswer ? .green.opacity(0.5) : .answersGrid.opacity(0.5))
                                    ...
                                    VStack {
                                        if animateViewsIn {
                                            Button {
                                                withAnimation(.easeOut(duration: 1)) {
                                                    tappedWrongAnswer.append(answer)
                                                }
                                                playWrongSound()
                                                game.questionScore -= 1
                                            } label: {
                                                Text(answer)
                                                    ...
                                                    .padding(10)
                                                    .frame(width: geo.size.width/2.15, height: 80)
                                                    .background(tappedWrongAnswer.contains(answer) ? .red.opacity(0.5) : .answersGrid.opacity(0.5))
                                                    .clipShape(.rect(cornerRadius: 25))
                                                    .scaleEffect(tappedWrongAnswer.contains(answer) ? 0.8 : 1)
                                            }
                                            .transition(.scale)
                                            .sensoryFeedback(.error, trigger: tappedWrongAnswer)
                                            .disabled(tappedWrongAnswer.contains(answer))
                                        }
                                    }
                                    .animation(.easeOut(duration: animateViewsIn ? 1 : 0).delay(animateViewsIn ? 1.5 : 0), value: animateViewsIn)
                                }
                    ...
                    VStack {
                        if tappedCorrectAnswer {
                            Text("\(game.questionScore)")
                                ...
                                .offset(x: movePointsToScore ? geo.size.width/2.3 : 0, y: movePointsToScore ? -geo.size.width/13 : 0)
